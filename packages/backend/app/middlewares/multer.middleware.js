...
const storage = multer.diskStorage({
    destination: (req, file, cb) => {
        ...
        if (req.body.parentId) {
            try {
                const parentFolder = FileSystem.findByIdAndUserId(
                    req.body.parentId,
                    userId
                )
        ...
        // Ensure the directory exists before multer tries to write
        // Use synchronous mkdirSync with recursive option
        try {
        ...
        if (fs.existsSync(fullFilePath)) {
            return cb(
                new multer.MulterError('File already exists!', file),
                false
            )
        }
