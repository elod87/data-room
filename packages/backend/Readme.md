...
1. Create a `.env` file and set your environment variables:
    ...
    - `CLIENT_URI` (optional, for CORS configuration)
...
2. Access the Swagger documentation: Open
   [http://localhost:3001/api-docs/](http://localhost:3001/api-docs/) in your browser to see all
...
Refer to the [Swagger Documentation](http://localhost:3001/api-docs/) for detailed request/response
formats.
...
│   ├── models/
│   │   └── FileSystem.model.js # In-memory storage model
│   └── routes/
...
- File metadata is stored **in-memory** and will be lost when the server restarts.
- Actual files are stored on disk in the `/public/uploads/` directory.
- This setup is suitable for development and MVP purposes.
